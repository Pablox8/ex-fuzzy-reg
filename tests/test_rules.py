...
def test_generate_partitions_returns_one_variable_per_column():
    data = np.random.rand(10, 4)
    partitions = utils.generate_partitions(data)

    assert len(partitions) == 4


def test_generate_partitions_uses_default_variable_names_when_not_provided():
    data = np.array([[0, 1]])
...
def test_generate_partitions_sets_domain_from_data_min_and_max():
...
def test_generate_partitions_creates_n_labels_per_variable():
    data = np.array([[1, 40],
                     [1.8, 75],
                     [1.6, 150],
                     [2.2, 85]])
    ...
    n_labels = 5
    partitions = utils.generate_partitions(data, n_labels)

    assert len(partitions[0].linguistic_variables) == n_labels
    assert len(partitions[1].linguistic_variables) == n_labels


def test_generate_partitions_assigns_linguistic_label_names_with_variable_name():
    label_names = ['height', 'weight']
    diff_values = ['low', 'medium', 'high']
    ...
    partitions = utils.generate_partitions(data, 3, label_names)

    for i in range(3):
        assert partitions[0].linguistic_variables[i].name == f"{diff_values[i]} height"
        assert partitions[1].linguistic_variables[i].name == f"{diff_values[i]} weight"


def test_generate_partitions_handles_constant_column():
    data = np.array([[5, 10],
                     [5, 20],
                     [5, 30]])
...
def test_generate_partitions_computes_correct_membership_parameters():
    data = np.array([[1, 40],
                     [1.8, 75],
                     [1.6, 150],
                     [2.2, 85]])

    partitions = utils.generate_partitions(data, 3)
